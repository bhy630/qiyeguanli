...
function init() {
	...
	$.ajax({
		url:'http://localhost:8080/EmpController?type=toUpdate',
		data:{eid:eid},
		dataType:'json',
		type:'post',
	...
	//得到下拉框的值 
	$.ajax({
		url:'http://localhost:8080/DeptController?type=listAll',
		dataType:'json',
		type:'post',
		data:'',
		async : true,
	...
	$.ajax({
		url:'http://localhost:8080/JsController?type=listAll',
		dataType:'json',
		type:'post',
		data:'',
		async : true,
...
function commitItem()
{
	...
	if(erealname.length==0)
		...
		{
		   var mypart = "emp.myjs.jid=" + jid + "&emp.mydept.pid=" + pid + "&emp.ename="+ename+"&emp.epsw="+epsw+"&emp.erealname="+erealname+"&emp.etel="+etel+"&emp.eaddress="+eaddress+"&emp.eremark="+eremark+"&emp.eid="+eid+"&emp.eflag="+eflag;
		   var i = layer.load(0);
		   $.post('http://localhost:8080/house2023/EmpController?type=update',mypart,function(mydata){
			 layer.close(i);
			 if(mydata==1)
				 {
