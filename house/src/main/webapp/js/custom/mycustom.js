...
function getAll(current) {
    ...
    $.ajax({
    	url:'http://localhost:8080/CustomController?type=list',
    	dataType:'json',
...
function outupdate()
{
	$(".myupp").live("click",function(){
		...
		layer.open({
			...
			shadeClose:true,
			area : [ '300px', '100px' ],
			content : '../custom/update.html?cid='+cid,
			skin : 'layui-layer-lan',
			shift: 4, //动画类型
			//弹出来后，设定层出现的位置
			success : function(layero, index) {
...
function mydel() {
	$(".mydel").live("click", function() {
		...
		layer.confirm('你确定要删除吗？', 
		       ...
		       , function() 
		          {
		    	      $.post('http://localhost:8080/CustomController?type=delete&cid='+cid,'',function(data){
	    	    	      if(data==1)
	    	    	    	  {
