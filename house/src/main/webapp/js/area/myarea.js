...
function getAll(current) {
	...
	$(".tablelist").empty();
	$(".tablelist").append("<thead><tr align='center'><th width='10%' align='center'>编号</th><th width='25%' align='center'>片区名称</th><th width='15%' align='center'>修改</th></tr></thead>");
	
    $.ajax({
    	url:'http://localhost:8080/AreaController?type=list',
    	dataType:'json',
    	...
    	success:function(mydata)
    	{
    		...
    		if(mydata=="9999")
    			...
    			$.each(mydata.list,function(index,xx){
    				{
    				    var srt='<tr><td>'+xx.aid+'</td>';
    				    srt+='<td>'+xx.aname+'</td>';
    				    srt+='<td><a href="javascript:void(0)" class="myupp">修改片区</a>';
...
function outupdate()
{
	$(".myupp").live("click",function(){
		...
		layer.open({
			...
			shadeClose:true,
			area : [ '300px', '100px' ],
			content : 'http://localhost:8080/area/update.html?aid='+aid,
			skin : 'layui-layer-lan',
			shift: 4, //动画类型
			//弹出来后，设定层出现的位置
			success : function(layero, index) {
...
function mydel() {
	$(".mydel").live("click", function() {
		...
		layer.confirm('你确定要删除吗？', 
		       ...
		       , function() 
		          {
		    	      $.post('http://localhost:8080/AreaController?type=delete&aid='+aid,'',function(data){
	    	    	      if(data==1)
	    	    	    	  {
