...
function getAll(current) {
    ...
    $.ajax({
    	url:'http://localhost:8080/SortController?type=list',
    	dataType:'json',
    			...
    			$.each(mydata.list,function(index,xx){
    				{
    				    ...
    				    srt+='<td>'+xx.sname+'</td>';
    				    srt+='<td><a href="javascript:void(0)" class="myupp">修改类型</a>';
...
function outupdate()
{
	$(".myupp").live("click",function(){
		...
		layer.open({
			...
			shadeClose:true,
			sort : [ '300px', '100px' ],
			content : 'http://localhost:8080/sort/update.html?sid='+sid,
			skin : 'layui-layer-lan',
			shift: 4, //动画类型
			//弹出来后，设定层出现的位置
			success : function(layero, index) {
...
function mydel() {
	$(".mydel").live("click", function() {
		...
		layer.confirm('你确定要删除吗？', 
		       ...
		       , function() 
		          {
		    	      $.post('http://localhost:8080/SortController?type=delete&sid='+sid,'',function(data){
	    	    	      if(data==1)
	    	    	    	  {
