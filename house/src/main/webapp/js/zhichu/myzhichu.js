...
function getAll(current) {
    ...
    $.ajax({
    	url:'http://localhost:8080/ZhichuController?type=list',
    	dataType:'json',
    	...
    	success:function(mydata)
    	{
    		if(mydata=="9999")
    			...
    			$.each(mydata.list,function(index,xx){
    				{
    				    var srt='<tr><td>'+xx.zid+'</td>';
    				    srt+='<td>'+xx.zmoney+'</td>';
    				    srt+='<td>'+xx.ztm+'</td>';
    				    srt+='<td style="text-align:left;">'+xx.zremark+'</td>';
    				    srt+='<td>'+xx.ztime+'</td>';
...
function outupdate()
{
	$(".myupp").live("click",function(){
		...
		var zid = $(this).parents("tr").children("td").eq(0).text();
		layer.open({
			type:2,
			title:'修改支出',
			fix:false,
			shadeClose:true,
			sr : [ '300px', '100px' ],
			content : '../zhichu/upp.html?zid='+zid,
			skin : 'layui-layer-lan',
			shift: 4, //动画类型
			//弹出来后，设定层出现的位置
			success : function(layero, index) {
...
function getSeAll()
{
	$(".seAll").click(function(){
		window.location.href="zhicAction_getSe.action";
	  /* $.ajax({
		   url:'sr_getSe.action',
		   dataType:'json',
		   type:'post',
