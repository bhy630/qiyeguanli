...
function getAll(current) {
    ...
    $.ajax({
    	url:'http://localhost:8080/BsController?type=list',
    	dataType:'json',
    	...
    	success:function(mydata)
    	{
    		...
    		$.each(mydata.list,function(index,xx){
    				{
    				    var srt='<tr><td>'+xx.bid+'</td>';
    				    srt+='<td>'+xx.haddress+'</td>';
    				    srt+='<td>'+xx.hfh+'</td>';
    				    srt+='<td style="text-align: left;">'+xx.bremark+'</td>';
    				    srt+='<td>'+xx.mtime+'</td>';
...
function outupdate()
{
	$(".myupp").live("click",function(){
		...
		var bid = $(this).parents("tr").children("td").eq(0).text();
		...
		layer.open({
			...
			shadeClose:true,
			sort : [ '300px', '100px' ],
			content : '../bs/update.html?bid='+bid,
			skin : 'layui-layer-lan',
			shift: 4, //动画类型
			//弹出来后，设定层出现的位置
			success : function(layero, index) {
