...
function getAll(current,flag) {
    ...
    $.ajax({
    	url:'http://localhost:8080/JsController?type=list',
    	dataType:'json',
    		...
    		if(mydata=="9999")
    			...
    			$.each(mydata.list,function(index,xx){

    				{
    				       ...
    				       srt+='<td>'+xx.jname+'</td>';
    				       srt+='<td><a href="javascript:void(0)" class="myupp">修改角色</a>';
...
function outupdate()
{
	$(".myupp").live("click",function(){
		...
		layer.open({
			...
			shadeClose:true,
			area : [ '300px', '300px' ],
			content : 'http://localhost:8080/myjs/update.html?jid='+jid,
			skin : 'layui-layer-lan',
			shift: 4, //动画类型
			//弹出来后，设定层出现的位置
			success : function(layero, index) {
...
function mydel() {
	$(".mydel").live("click", function() {
		...
		layer.confirm('你确定要删除吗？',
		       ...
		       , function()
		          {
		    	      $.post('http://localhost:8080/JsController?type=delete&jid='+jid,'',function(data){
	    	    	      if(data==1)
	    	    	    	  {
