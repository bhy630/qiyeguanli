...
function getAll(current) {
	...
	$(".tablelist").empty();
	$(".tablelist").append("<thead><tr style='text-align: center;'><th  align='center'>编号</th><th  align='center'>标题</th><th  align='center'>内容</th><th  align='center'>时间</th><th  align='center'>修改</th></tr></thead>");
	
    $.ajax({
    	url:'http://localhost:8080/ZcController?type=list',
    	dataType:'json',
    			...
    			//分页需要的参数
    				{
    				    ...
    				    srt+='<td style="text-align: left"><a href="#" class="look">'+xx.ctitle+'</A></td>';
    				    srt+='<td style="text-align: left;">'+xx.cremark+'</td>';
    				    srt+='<td>'+xx.ctime+'</td>';
    				    
    				    srt+='<td><a href="#" class="myupp">修改</a></td></tr>';
...
function addEmp(){
	...
	$(".addPart").bind("click",function() {
		window.location.href='http://localhost:8080/zc/add.html';
		/*layer.open( {
			type : 2,
			title : "发布新政",
			fix : false,
			shadeClose : true,
			area : [ '300px', '100px' ],
			content : $webName+'/zc/add.html',
			skin : 'layui-layer-lan',
			shift: 4, 动画类型			弹出来后，设定层出现的位置			success : function(layero, index) {
				layer.style(index, {
					width : '800px',
					height : '500px',
					top : '100px',
...
function outupdate()
{
	$(".myupp").live("click",function(){
		var cid = $(this).parents("tr").children("td").eq(0).text();
		window.location.href='http://localhost:8080/zc/add.html?cid='+cid;
	/*	layer.open({
			type:2,
			title:'修改政策',
			fix:false,
			shadeClose:true,
			area : [ '300px', '100px' ],
			content : $webName+'/zc/upp.html?cid='+cid,
			skin : 'layui-layer-lan',
			shift: 4, 动画类型			弹出来后，设定层出现的位置			success : function(layero, index) {
				layer.style(index, {
					width : '800px',
