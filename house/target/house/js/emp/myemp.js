...
function getAll(current,flag) {
    ...
    $.ajax({
    	url:'http://localhost:8080/EmpController?type=list',
    	dataType:'json',
...
function outupdate()
{
	$(".myupp").live("click",function(){
		...
		layer.open({
			...
			shadeClose:true,
			area : [ '300px', '300px' ],
			content : 'http://localhost:8080/emp/update.html?eid='+eid,
			skin : 'layui-layer-lan',
			shift: 4, //动画类型
			//弹出来后，设定层出现的位置
			success : function(layero, index) {
...
function mydel() {
	$(".mydel").live("click", function() {
		...
		layer.confirm('你确定要删除吗？', 
		       ...
		       , function() 
		          {
		    	      $.post('http://localhost:8080/EmpController?type=delete&eid='+eid,'',function(data){
	    	    	      if(data==1)
	    	    	    	  {
