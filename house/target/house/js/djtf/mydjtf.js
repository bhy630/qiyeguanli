...
function getAll(current,flag) {
	...
	$(".tablelist").empty();
	$(".tablelist").append("<thead><tr align='center'><th  align='center'>编号</th><th align='center'>房屋地址</th><th  align='center'>房号</th><th  align='center'>客户姓名</th><th  align='center'>客户电话</th><th  align='center'>登记时间</th><th>下次收租日期</th><th>办理停租手续</th></tr></thead>");
	$.ajax({
    	url:'http://localhost:8080/DjtfController?type=list',
    	dataType:'json',
    			...
    			if(xx.mid!=null)
    				{
    				    ...
    				    srt+='<td><a href="#" class="myupp">办理停租手续</a></td>';
...
function outupdate()
{
	$(".myupp").live("click",function(){
		...
		layer.open({
			...
			shadeClose:true,
			area : [ '300px', '300px' ],
			content : $webName+'/djtf/upp.jsp?mid='+mid,
			skin : 'layui-layer-lan',
			shift: 4, //动画类型
			//弹出来后，设定层出现的位置
			success : function(layero, index) {
