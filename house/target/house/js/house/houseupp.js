...
function init() {
	...
	$.ajax({
		url:'http://localhost:8080/HouseController?type=toUpdate',
		data:{hid:hid},
		dataType:'json',
		type:'post',
		success:function(mydata)
		{
			...
			$("#pic01").attr('src',mydata.pic1);
			$("#pic02").attr('src',+mydata.pic2);
			$("#pic03").attr('src',mydata.pic3);
			sid=mydata.sid;
			aid=mydata.aid;
			hid=mydata.hid
		}
	...
	//得到下拉框的值 
	$.ajax({
		url:'http://localhost:8080/SortController?type=listAll',
		dataType:'json',
		type:'post',
		data:'',
		async : true,
...
function commitItem()
{
	...
	if((x1.length==0&&x2.length!=0 && x3.length!=0)||(x1.length==0&&x2.length==0 &&x3.length!=0)||(x1.length==0&&x2.length!=0 && x3.length==0))
		...
		{
		$.ajaxFileUpload({
	    url:'http://localhost:8080/HouseController?type=update',
