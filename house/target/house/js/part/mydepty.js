...
function getAll(current,flag) {
	...
	if(flag==0)
		{
		  $(".tablelist").append("<thead><tr align='center'><th width='10%' align='center'>编号</th><th width='25%' align='center'>部门名称</th><th width='35%' align='center'>部门备注</th><th width='15%'>运营部门</th><th width='15%'>操作</th></tr></thead>");
		}
	else
		{
		   $(".tablelist").append("<thead><tr align='center'><th width='10%' align='center'>编号</th><th width='25%' align='center'>部门名称</th><th width='35%' align='center'>部门备注</th><th width='15%' align='center'>已撤销部门</th><th width='15%'>操作</th></tr></thead>");
		}
    $.ajax({
    	url:'http://localhost:8080/DeptController?type=list',
    	dataType:'json',
    			 ...
    			 $.each(mydata.list,function(index,xx){
    				{
    				    	    ...
    				    	    var srt='<tr><td>'+xx.pid+'</td>';
		    				    srt+='<td>'+xx.pname+'</td>';
		    				    srt+='<td>'+xx.premark+'</td>';
    				    	    srt+='<td><a href="#" class="backdel">' + xx.pflag + '</a></td>';
    				    	    srt+='<td><a href="javascript:void(0)" class="myupp">修改</a>';
								srt+='<a href="javascript:void(0)" class="mydel">删除</a></td>';
    				    	    srt+='</tr>'
...
function outupdate()
{
	$(".myupp").live("click",function(){
		...
		layer.open({
			...
			shadeClose:true,
			area : [ '300px', '300px' ],
			content :'http://localhost:8080/part/update.html?pid='+pid,
			skin : 'layui-layer-lan',
			shift: 4, //动画类型
			//弹出来后，设定层出现的位置
			success : function(layero, index) {
...
function mydel() {
	$(".mydel").live("click", function() {
		...
		layer.confirm('你确定要删除吗？', 
		       ...
		       , function() 
		          {
		    	      $.post('http://localhost:8080/DeptController?type=delete&pid='+pid,'',function(data){
	    	    	      if(data==1)
	    	    	    	  {
